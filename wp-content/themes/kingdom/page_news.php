...
<div class="news">
	...
	global $post,$post_id;
		...
		$count_post = 0;
		query_posts( array( 'posts_per_page' => '-1', 'post_type' => 'post', 'category_name' => "$cs_news_cat_db" ,'post_status' => 'publish') );
			while ( have_posts()) : the_post();
				$count_post++;
			endwhile;
			if ( $cs_news_pagination_db == "Single Page" ) $cs_news_num_post_db = -1;
	
			query_posts( array( 'posts_per_page' => "$cs_news_num_post_db", 'paged' => $_GET['page_id_all'], 'category_name' => "$cs_news_cat_db" ,'post_status' => 'publish') ); 
			$counter_news = 0;
			while ( have_posts()) : the_post();
				$counter_news++;
