...
<?php if ( have_posts() )	the_post(); ?>
		...
		<div class="<?php if(isset($cs_layout)) echo $cs_layout;?>">           
            ...
            <div class="blog">
					...
					<?php
					wp_reset_query();
						$general_category = single_term_title( '', false );						
						$newterm = get_term_by('slug', $general_category, 'category');
							if ( empty($_GET['page_id_all']) ) $_GET['page_id_all'] = 1;
							if ( $cs_pagination == "Single Page" ) $cs_pagination = -1;
						$newpost = 'posts_per_page=-1&cat='.$newterm->term_id.'&post_type=post&post_status=publish';
						$newquery_tag = new WP_Query($newpost);
							$count_post = 0;
						while ( $newquery_tag->have_posts() ) : $newquery_tag->the_post(); 
								$count_post++;						
						endwhile;
						$new = 'posts_per_page='.$record_per_page.'&cat='.$newterm->term_id.'&paged='.$_GET["page_id_all"].'&post_status=publish';
					...
					?>
                        ...
                        <div class="post-opts">
                            ...
                            <p class="cat"><?php $categories = get_the_terms( $post->ID, 'category' );
									if($categories != ''){
										$couter_comma = 0;
