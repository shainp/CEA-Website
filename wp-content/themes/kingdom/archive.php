		...
		<div class="<?php if(isset($cs_layout)) echo $cs_layout;?>">          
            <div class="box">
				<h2 class="heading">
					<?php 
					wp_reset_query();
						if ( is_day() ) : printf( __( 'Daily Archives: <span>%s</span>', 'PixFill' ), get_the_date() ); 
						elseif ( is_month() ) :  printf( __( 'Monthly Archives: <span>%s</span>', 'PixFill' ), get_the_date( _x( 'F Y', 'monthly archives date format', 'PixFill' ) ) );
						elseif ( is_year() ) : printf( __( 'Yearly Archives: <span>%s</span>', 'PixFill' ), get_the_date( _x( 'Y', 'yearly archives date format', 'PixFill' ) ) );
               ...
               <div class="box-in">	                    
					<?php				
						if ( empty($_GET['page_id_all']) ) $_GET['page_id_all'] = 1;
							...
							$newpost = 'posts_per_page=-1&monthnum='.get_the_time('m').'&post_type=post&post_status=publish';
							...
							$new = 'posts_per_page='.$record_per_page.'&monthnum='.get_the_time('m').'&paged='.$_GET["page_id_all"].'&post_status=publish';
                ...
                if(cs_pagination($count_post, $record_per_page,$qrystr) <> ''){
                    ...
                    if ( $cs_pagination == "Show Pagination" and $record_per_page > 0 ) {
                        echo "<div class='pagination'><h5>Pages</h5><ul>";
                            if ( isset($_GET['monthnum']) ) $qrystr = "&monthnum=".get_the_time('m');								
