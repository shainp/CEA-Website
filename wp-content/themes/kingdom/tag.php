...
<?php if ( have_posts() )	the_post(); ?>
		...
		<div class="<?php if(isset($cs_layout)) echo $cs_layout;?>">            
            <div class="blog">
                	<h2 class="heading"><?php echo 'Tag :  '.single_tag_title( '', false );?></h2>
                    <div class="box-in">	  
            <?php
			wp_reset_query();
                if ( empty($_GET['page_id_all']) ) $_GET['page_id_all'] = 1;
							...
							$general_category = single_tag_title( '', false );						
							$newterm = get_term_by('slug', $general_category, 'post_tag');
							$newpost = 'posts_per_page=-1&tag='.$newterm->slug.'&post_type=post&post_status=publish';
							$newquery_dd = new WP_Query($newpost);
								$count_post = 0;
									while ( $newquery_dd->have_posts() ) : $newquery_dd->the_post(); 
											$count_post++;						
									endwhile;
							$new = 'posts_per_page='.$record_per_page.'&tag='.$newterm->slug.'&paged='.$_GET["page_id_all"].'&post_status=publish';
